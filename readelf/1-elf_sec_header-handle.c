 ...
 *
 * this function serves as the primary interface for parsing and
 * validating ELF files;
 * it orchestrates the calls to helper functions for specific validations
 * and fetch operations;
 * the function will also initialize data structures to be used in further
 * processing;
 *
...
void handle_elf(char *mem, size_t fileSize, int is_64bit, int is_big_endian)
{
	...
	get_section_header_info(elf_header, fileSize, is_64bit, is_big_endian,
				&e_shoff, &count);

...
/**
 * validate_file_size - program that validates the size of the ELF file based on its architecture
 *
 * this function checks if the given ELF file size meets the minimum requirements
 * for a valid ELF-32 or ELF-64 file;
...
/**
 * get_section_header_info - program that retrieves and validates the section header table information from the ELF header
 *
 * this function populates the section header offset and the section header count
 * from the ELF header;
 * it also performs validation to make sure the section header table does not extend
...
{
    *e_shoff = is_64bit ? ((Elf64_Ehdr *)elf_header)->e_shoff : ((Elf32_Ehdr *)elf_header)->e_shoff;
    *count = is_64bit ? ((Elf64_Ehdr *)elf_header)->e_shnum : ((Elf32_Ehdr *)elf_header)->e_shnum;
...
/**
 * get_string_table_index - program that fetches the index of the section header string table from the ELF header
 *
 * this function retrieves the index of the section header string table which holds
...
{
    *e_shstrndx = is_64bit ? ((Elf64_Ehdr *)elf_header)->e_shstrndx : ((Elf32_Ehdr *)elf_header)->e_shstrndx;

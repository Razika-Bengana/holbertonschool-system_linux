 ...
 *
 * Return: a pointer to the mapped memory if successful, or MAP_FAILED if an error occurs
 */
 ...
 *
 * this function determines whether the file is ELF32 or ELF64 and calls the corresponding function
 ...
 *
 * 1. checks if the filename has been provided as a command-line argument. If not, exits with an error
 * 2. opens the specified file and obtains its file descriptor. If the file cannot be opened, exits with an error
 * 3. retrieves file metadata using fstat and validates it. If this step fails, closes the file and exits with an error
 * 4. maps the file into memory for easier manipulation. If the mapping fails, closes the file and exits with an error
 * 5. delegates the handling of the ELF file based on its type (32-bit or 64-bit) and endianness
 * 6. unmaps the file from memory and closes the file descriptor
 *
 * @argc: the number of command-line arguments
 * @argv: an array of command-line arguments
 *
 * Return: 0 upon successful execution, and if an error occurs at any stage, returns 1
 */

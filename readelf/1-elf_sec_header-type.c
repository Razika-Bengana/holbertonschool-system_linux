...
/**
 * section_type_to_string - program that converts an ELF section type constant to its corresponding string representation
 *
 * given a uint32_t type value that represents an ELF section type, this function
 * returns a pointer to a string that describes the section type in human-readable form;
 *
 ...
 * - SHT_INIT_ARRAY, SHT_FINI_ARRAY, SHT_PREINIT_ARRAY: array of pointers to initialization and termination functions;
 * - SHT_GROUP: section group;
 * - SHT_SYMTAB_SHNDX: extended section indeces;
 * - SHT_NUM: number of defined types;
 * - SHT_PROGBITS: holds program data;
 * - Vendor-specific types (VERSYM, VERDEF, VERNEED, GNU_HASH, LOOS, LOUSER, etc.)
 *
 * @type: a uint32_t value representing the ELF section type
 *
 * Return: a pointer to a string describing the section type. Returns "UNKNOWN" for unrecognized types
 */

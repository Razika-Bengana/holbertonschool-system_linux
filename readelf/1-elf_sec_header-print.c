...
/**
 *  print_common_headers - program that prints the common header lines for ELF section headers
 *
...
/**
 * print_flags_section - program that prints the legend explaining the flags in the section headers
 *
 * @is_64bit: indicates if the ELF file is 64-bit
 *
 * Return: nothing (void)
...
/**
 * swap_and_assign_section_data_64 - program that converts and swaps (if necessary) 64-bit ELF section headers
...
/**
 * handle_64bit_section - program that processes and prints the section headers for a 64-bit ELF file
 *
...
void handle_64bit_section(SectionHeaderInfo *info, int isLittleEndian)
{
    ...
    for (int i = 0; i < info->count; i++)
    {
        ...
        swap_and_assign_section_data_64(section, &converted, isLittleEndian);
...
/**
 * swap_and_assign_section_data_32 - program that converts and swaps (if necessary) 32-bit ELF section headers
 * from the original ELF format to a more manageable format
 *
...
/**
 * handle_32bit_section - program that processes and prints the section headers for a 32-bit ELF file
 *
 * @info: a struct containing all the necessary info about the ELF file
 * @isLittleEndian: indicates if the ELF file uses little-endian encoding
 *
...
void handle_32bit_section(SectionHeaderInfo *info, int isLittleEndian)
{
    ...
    for (int i = 0; i < info->count; i++)
    {
        ...
        swap_and_assign_section_data_32(section, &converted, isLittleEndian);

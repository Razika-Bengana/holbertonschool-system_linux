...
/**
 * swap_fields_64 - program that swaps the byte order of various fields in a 64-bit ELF header
 *
 ...
 *
 * this function reads the `e_ident[EI_DATA]` field from the ELF header to determine
 * the data encoding of the ELF file;
 * it then prints this information to standard output;
 * additionally, if the data encoding in the ELF header does not match the endianness of
 * the system running this code, it performs byte-swapping on several fields in the header;
 *
 * @header: a pointer to the ELF header structure, which could be either Elf64_Ehdr or Elf32_Ehdr
 * @is_elf64: a flag to specify whether the ELF file is 64-bit (1) or 32-bit (0)
 *
 * Return: nothing (void)
 */

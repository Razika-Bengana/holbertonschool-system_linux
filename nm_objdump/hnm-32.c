 ...
 * @string_table: a pointer to the beginning of the string table, which contains the names of the symbols
...
/**
 * process_elf_file32 - program that processes a 32-bit ELF file located at the given file path
 *
 * this function opens the file, reads the ELF header, and verifies the ELF format and endianness;
 * it then reads the section headers to locate the symbol table and the string table;
 * afterward, it reads the symbol table and string table from the file and calls print_symbol_table32
 * to print the symbol information
 *
